...
* WPCD ID: 101
*
* Bitbucket Plugin URI: https://bitbucket.org/cardemons/car-demon/
...
/**
 * Define CD_NO_AFFILIATES as true to turn off affiliate sales staff feature by default
*/
if ( ! defined( 'CD_NO_AFFILIATES' ) ) {
	define( 'CD_NO_AFFILIATES', true );
}
 ...
 *     - Document functions
 */
require_once( 'includes/cd-page-navi.php' );

/**
 * Functions for turning off Gutenberg support for the near future
 *
 */
require_once( 'includes/cd-gutenberg.php' );

/**
 * Functions and files for adding Car Demon Blocks to Gutenberg
 *
 */
require_once( 'blocks/cd-blocks.php' );
...
function cd_default_options() {
	...
	$cd_options['hide_updates'] = 'No';
	update_option( 'car_demon_options', $cd_options );
}

/**
 * This function provides the ability to disable updating Car Demon.
 * Option was added in preparation of future release changes
 */
function cd_disable_plugin_updates( $value ) {
	global $car_demon_options;
	if ( isset( $car_demon_options['hide_updates'] ) && __( 'Yes', 'car-demon' ) === $car_demon_options['hide_updates'] ) {
		unset( $value->response['car-demon/car-demon.php'] );
	}
	return $value;
}
add_filter( 'site_transient_update_plugins', 'cd_disable_plugin_updates' );
