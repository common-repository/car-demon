...
function car_demon_query_search( $query = array() ) {
	...
	/*
		TO DO: Add all meta fields and handle in query if search is sent for them
	*/

	//= Correct legacy field names
	$get = cd_fix_deprecated_query_items( $get );
	...
	if ( isset( $get['car'] ) ) {
		...
		if ( isset( $get['search_mileage'] ) ) {
			if ( $get['search_mileage'] ) {
				$miles = sanitize_text_field( $get['search_mileage'] );
		...
		if ( isset( $get['search_transmission'] ) ) {
			if ( $get['search_transmission'] ) {
				$trans = sanitize_text_field( $get['search_transmission'] );
				$meta_query = array_merge( $meta_query, array( array( 'key' => '_transmission_value','value' => $trans, 'compare' => '=', 'type' => 'text' ) ) );
			}
		}
		if ( isset( $get['search_price_min'] ) ) {
			$min_price = sanitize_text_field( $get['search_price_min'] );
		} else {
			$min_price = '';
		}
		if ( isset( $get['search_price_max'] ) ) {
			$max_price = sanitize_text_field( $get['search_price_max'] );
		} else {
			$max_price = '';
		}
		...
		if ( isset( $get['search_mileage_min'] ) ) {
			if ( empty( $get['search_mileage_min'] ) ) {
				$get['search_mileage_min'] = 1;
			}
			$min_mileage = sanitize_text_field( $get['search_mileage_min'] ) - 1;
		} else {
			$min_mileage = '';
		}
		if ( isset( $get['search_mileage_max'] ) ) {
			if ( empty( $get['search_mileage_max'] ) ) {
				$get['search_mileage_max'] = 1;
			}
			$max_mileage = sanitize_text_field( $get['search_mileage_max'] ) + 1;
		} else {
			$max_mileage = '';
		}
		...
		if ( isset( $get['search_year_min'] ) ) {
			$min_year = sanitize_text_field( $get['search_year_min'] );
		} else {
			$min_year = '';
		}
		if ( isset( $get['search_year_max'] ) ) {
			$max_year = sanitize_text_field( $get['search_year_max'] ) + 1;
		} else {
			$max_year = '';
		}
		...
		if ( ! empty( $min_year ) ) {
			if ( $min_year == $max_year ) {
				...
				unset( $get['search_year_min'] );
				unset( $get['search_year_max'] );
				$min_year = '';
				$max_year = '';
			}
		}
		...
		if ( isset( $get['search_body_style'] ) ) {
			if ( $get['search_body_style'] ) {
				if ( strpos( $get['search_body_style'], '|' ) !== false ) {
					$body_style_array = explode( '|', $get['search_body_style'] );
					$body_tax = array();
					...
					$search_body_style = sanitize_text_field( $get['search_body_style'] );
					$car_query = array_merge( $car_query, array( 'vehicle_body_style' => $search_body_style ) );
				}
			}
		}
		...
		if ( isset( $get['search_location'] ) ) {
			if ( $get['search_location'] ) {
				if ( strpos( $get['search_location'], '|' ) !== false ) {
					$location_array = explode( '|', $get['search_location'] );
				} else {
					$location_array = array( $get['search_location'] );
				}

				$location_tax = array();
				$location_tax['taxonomy'] = 'vehicle_location';
				$location_tax['terms'] = $location_array;
				$location_tax['field'] = 'slug';
				$location_tax['compare'] = '=';
				$location_tax['type'] = 'string';
				unset($car_query['search_location']);
				$car_query['tax_query'][] = $location_tax;
	...
	if ( isset( $car_demon_options['drop_down_sort'] ) ) {
		if ( $car_demon_options['drop_down_sort'] == 'Yes' ) {
			...
			foreach ( $output as $key => $value ) {
				if ( ! empty( $value ) ) {
					$key = wp_strip_all_tags( $key, true );
...
/**
 * Replace deprecated search fields with their newer versions
 *
 * @param $get the array of the search items being used in the vehicle query
 *
 */
function cd_fix_deprecated_query_items( $get ) {

	//= Old field name on the left and new field name on the right
	$deprecated = array (
		'search_mileage' => 'search_mileage',
		'search_tran' => 'search_transmission',
		'search_dropdown_Min_price' => 'search_price_min',
		'search_dropdown_Max_price' => 'search_price_max',
		'search_dropdown_mileage_Min' => 'search_mileage_min',
		'search_dropdown_mileage_Max' => 'search_mileage_max',
		'search_dropdown_body' => 'search_body_style',
		'search_dropdown_Min_years' => 'search_year_min',
		'search_dropdown_Max_years' => 'search_year_max',
	);

	foreach( $deprecated as $old=>$new ) {
	
		if ( isset( $get[ $old ] ) ) {
			$get[ $new ] = sanitize_text_field( $get[ $old ] );
		}
	
	}

	$get = apply_filters( 'cd_fix_deprecated_query_items_filter', $get );
	return $get;
}
?>