...
function cd_save_car( $post_id ) {
	...
	foreach( $vin_query_decode as $name => $val ) {
		if ( empty( $val ) ) {
			unset( $vin_query_decode[ $name ] );
		}
	}
	...
	if ( isset( $_POST['post_title'] ) ) {
		...
		$vin_query_decode['title'] = $post_title;
	}
	
	if ( isset( $_POST['post_content'] ) ) {
		$post_content = $_POST['post_content'];
		$vin_query_decode['post_content'] = sanitize_text_field( $post_content );
	}
