...
function car_demon_admin_car_scripts() {
	...
	$no_photo = CAR_DEMON_PATH . 'images/no_photo.gif';
	if ( defined( 'CD_CUSTOM_NO_PHOTO' ) ) {
		$no_photo = CD_CUSTOM_NO_PHOTO;
	}
	$photo_box = cd_image_box( 'POST_ID', 'CNT', 'THUMBNAIL', 'SRC_ID', 'TYPE' );
	wp_register_script( 'car-demon-admin-js', plugins_url() . '/car-demon/admin/js/car-demon-admin.js', array( 'jquery','jquery-ui-core', 'jquery-ui-sortable', 'jquery-ui-droppable' ), CAR_DEMON_VER, true );
	wp_localize_script( 'car-demon-admin-js', 'cdAdminParams', array(
		...
		'bad_image_links' => __( 'The list of image URLs you entered was not valid. Please check them and try again.', 'car-demon' ),
		'no_photo' => $no_photo,
		'non_numeric_price' => $non_numeric_price,
		'photo_box' => $photo_box,
		'str_manage_vehicle_photos' => __( 'Manage Vehicle Photos', 'car-demon' ),
		'str_attach_photos_to_vehicle' => __( 'Attach Photos to Vehicle', 'car-demon' ),
		'remove_image_msg' => __( 'Are you sure you want to delete this image?', 'car-demon' ),
	));
...
add_action( "wp_ajax_cd_update_image_order", "cd_update_image_order" );
add_action( "wp_ajax_nopriv_cd_update_image_order", "cd_update_image_order" );
add_action( "wp_ajax_cd_update_main_image", "cd_update_main_image" );
add_action( "wp_ajax_nopriv_cd_update_main_image", "cd_update_main_image" );
add_action( "wp_ajax_cd_update_image_links", "cd_update_image_links" );
add_action( "wp_ajax_nopriv_cd_update_image_links", "cd_update_image_links" );

function cd_update_image_links() {
	if ( ! is_admin() ) {
		return;
	}
	...
	$nonce = $_POST['nonce'];
	if ( ! wp_verify_nonce( $nonce, 'cd_update_image_links_nonce' ) ) {
		echo __( 'Nonce check failed - no changes saved.', 'car-demon' );
		exit();
	}
	$post_id = sanitize_text_field( $_POST['post_id'] );
	$image_links = sanitize_text_field( $_POST['image_links'] );
	update_post_meta( $post_id, '_images_value', $image_links );
	echo __( 'Image links updated', 'car-demon' );
	exit();
}

function cd_update_main_image() {
	if ( ! is_admin() ) {
		return;
	}
	if ( ! current_user_can( 'manage_options' ) ) {
	...
	echo __( 'Main image updated', 'car-demon' );
	exit();
}

function cd_update_image_order() {
	if ( ! is_admin() ) {
		return;
	}
	if ( ! current_user_can( 'manage_options' ) ) {
	...
	if ( $total_links > 0 ) {
		...
		update_post_meta( $post_id, '_images_value', $linked_list );
	}
	...
	echo __( 'Image order updated', 'car-demon' );
	exit();
}
...
function car_demon_admin_update() {
	...
	$post_id = sanitize_text_field( $_POST['post_id'] );
	$nonce = $_POST['nonce'];
	if ( ! wp_verify_nonce( $nonce, 'car_demon_admin_update_nonce_' . $post_id ) ) {
		echo __( 'Nonce check failed - no changes saved.', 'car-demon' );
		exit();
	}
...
function car_demon_options() {
	...
	$default['dl_pagination'] = '#cd-nav-below.inventory_nav_bottom';
	$default['dl_next'] = '.ul li .nextpostslink';
	//= Content Replacement
	...
	$default['hide_updates'] = 'No';

	$car_demon_options = array();
	$car_demon_options = get_option( 'car_demon_options', $default );
	$car_demon_options = wp_parse_args( $car_demon_options, $default );
...
function car_demon_settings_options_do_page() {
	if ( isset( $_POST['add_page'] ) ) {
		...
		echo '<h2><a href="' . $link . '" target="_blank">New ' . $title . ' Page</a></h2>';
	}
	// in Welcome.php
...
function car_demon_settings_form() {
	...
	if ( isset( $_POST['reset_car_demon'] ) ) {
		$update_fail = true;
		if ( isset( $_POST['cd_admin_nonce'] ) ) {
			$nonce = $_POST['cd_admin_nonce'];
			if ( wp_verify_nonce( $nonce, 'cd_admin_nonce' ) ) {
				$update_fail = false;
				reset_car_demon();
			}
		}
		...
		if( isset( $_POST['update_car_demon']) == 1) {
			...
			if ( $update_fail ) {
				echo '<h3 style="color:#b00;">';
					echo __( 'Your settings failed to update - bad nonce sent.', 'car-demon' );
				echo '</h3>';
			}
		}
	}
	...
	echo '<form action="" method="post">';
		...
		echo '<input type="hidden" name="cd_admin_nonce" id="cd_admin_nonce" value="' . $nonce . '" />';
		...
		echo '<fieldset class="cd_admin_group">';
			...
			echo '<span class="cd_option_group">';
				...
				echo '<hr />';
				if ( defined( 'CD_CR_OPTIONS' ) && CD_CR_OPTIONS ) {
					echo '<fieldset>';
				...
				echo '<fieldset>';
					echo __( 'Hide Car Demon Updates.', 'car-demon');
					echo '<br />';
					echo '<select class="" name="hide_updates">';
						echo '<option value="' . __( 'No', 'car-demon' ) . '"' . ( __( 'No', 'car-demon' ) === $car_demon_options['hide_updates'] ? ' selected' : '' ) . '>' . __( 'No', 'car-demon' ) . '</option>';
						echo '<option value="' . __( 'Yes', 'car-demon' ) . '"' . ( __( 'Yes', 'car-demon' ) === $car_demon_options['hide_updates'] ? ' selected' : '' ) . '>' . __( 'Yes', 'car-demon' ) . '</option>';
					echo '</select>';
				echo '</fieldset>';
				echo '<div class="cd_welcome_clear"></div>';
				echo '<hr />';
...
function update_car_demon_settings() {
	...
	if ( isset( $_POST['currency_symbol'] ) ) $new['currency_symbol'] = sanitize_text_field( $_POST['currency_symbol'] );
	if ( isset( $_POST['currency_symbol_after'] ) ) $new['currency_symbol_after'] = sanitize_text_field( $_POST['currency_symbol_after'] );
	if ( isset( $_POST['vinquery_id'] ) ) $new['vinquery_id'] = sanitize_text_field( $_POST['vinquery_id'] );
	if ( isset( $_POST['vinquery_type'] ) ) $new['vinquery_type'] = sanitize_text_field( $_POST['vinquery_type'] );
	if ( isset( $_POST['use_about'] ) ) $new['use_about'] = sanitize_text_field( $_POST['use_about'] );
	if ( isset( $_POST['adfxml'] ) ) $new['adfxml'] = sanitize_text_field( $_POST['adfxml'] );
	if ( isset( $_POST['use_compare'] ) ) $new['use_compare'] = sanitize_text_field( $_POST['use_compare'] );
	if ( isset( $_POST['secure_finance'] ) ) $new['secure_finance'] = sanitize_text_field( $_POST['secure_finance'] );
	if ( isset( $_POST['use_theme_files'] ) ) $new['use_theme_files'] = sanitize_text_field( $_POST['use_theme_files'] );
	if ( isset( $_POST['dynamic_load'] ) ) $new['dynamic_load'] = sanitize_text_field( $_POST['dynamic_load'] );
	if ( isset( $_POST['dynamic_ribbons'] ) ) $new['dynamic_ribbons'] = sanitize_text_field( $_POST['dynamic_ribbons'] );
	if ( isset( $_POST['mobile_chat_code'] ) ) $mobile_chat_code = sanitize_text_field( $_POST['mobile_chat_code'] );
	$mobile_chat_code = '';
	$mobile_chat_code = str_replace( "\'", "'", $mobile_chat_code );
	$mobile_chat_code = str_replace( '\"', '"', $mobile_chat_code );
	$mobile_chat_code = str_replace( '\\', '', $mobile_chat_code );	
	$new['mobile_chat_code'] = $mobile_chat_code;
	if ( isset( $_POST['custom_header_type'] ) ) $new['custom_header_type'] = sanitize_text_field( $_POST['custom_header_type'] );
	...
	if ( isset( $_POST['mobile_header'] ) ) $new['mobile_header'] = sanitize_text_field( $_POST['mobile_header'] );
	if ( isset( $_POST['validate_phone'] ) ) $new['validate_phone'] = sanitize_text_field( $_POST['validate_phone'] );
	if ( isset( $_POST['before_listings'] ) ) $new['before_listings'] = sanitize_text_field( $_POST['before_listings'] );
	if ( isset( $_POST['use_post_title'] ) ) $new['use_post_title'] = sanitize_text_field( $_POST['use_post_title'] );
	if ( isset( $_POST['show_sold'] ) ) $new['show_sold'] = sanitize_text_field( $_POST['show_sold'] );
	if ( isset( $_POST['cc_admin'] ) ) $new['cc_admin'] = sanitize_text_field( $_POST['cc_admin'] );
	if ( isset( $_POST['drop_down_sort'] ) ) $new['drop_down_sort'] = sanitize_text_field( $_POST['drop_down_sort'] );
	if ( isset( $_POST['do_sort'] ) ) $new['do_sort'] = sanitize_text_field( $_POST['do_sort'] );
	if ( isset( $_POST['sort_price'] ) ) $new['sort_price'] = sanitize_text_field( $_POST['sort_price'] );
	if ( isset( $_POST['sort_miles'] ) ) $new['sort_miles'] = sanitize_text_field( $_POST['sort_miles'] );
	if ( isset( $_POST['hide_tabs'] ) ) $new['hide_tabs'] = sanitize_text_field( $_POST['hide_tabs'] );
	if ( isset( $_POST['popup_images'] ) ) $new['popup_images'] = sanitize_text_field( $_POST['popup_images'] );
	if ( isset( $_POST['custom_options'] ) ) $new['custom_options'] = sanitize_text_field( $_POST['custom_options'] );
	if ( isset( $_POST['use_form_css'] ) ) $new['use_form_css'] = sanitize_text_field( $_POST['use_form_css'] );
	if ( isset( $_POST['use_vehicle_css'] ) ) $new['use_vehicle_css'] = sanitize_text_field( $_POST['use_vehicle_css'] );
	if ( isset( $_POST['title_trim'] ) ) $new['title_trim'] = sanitize_text_field( $_POST['title_trim'] );
	if ( isset( $_POST['cars_per_page'] ) ) $new['cars_per_page'] = sanitize_text_field( $_POST['cars_per_page'] );
	//= Legacy Specs option
	
	if ( isset( $_POST['show_custom_specs'] ) ) $new['show_custom_specs'] = sanitize_text_field( $_POST['show_custom_specs'] );
	//=Sidebars
	if ( isset( $_POST['vehicle_sidebar_class'] ) ) $new['vehicle_sidebar_class'] = sanitize_text_field( $_POST['vehicle_sidebar_class'] );
	if ( isset( $_POST['left_list_sidebar'] ) ) $new['left_list_sidebar'] = sanitize_text_field( $_POST['left_list_sidebar'] );
	if ( isset( $_POST['right_list_sidebar'] ) ) $new['right_list_sidebar'] = sanitize_text_field( $_POST['right_list_sidebar'] );
	if ( isset( $_POST['vehicle_container'] ) ) $new['vehicle_container'] = sanitize_text_field( $_POST['vehicle_container'] );
	if ( isset( $_POST['left_vehicle_sidebar'] ) ) $new['left_vehicle_sidebar'] = sanitize_text_field( $_POST['left_vehicle_sidebar'] );
	if ( isset( $_POST['right_vehicle_sidebar'] ) ) $new['right_vehicle_sidebar'] = sanitize_text_field( $_POST['right_vehicle_sidebar'] );
	if ( isset( $_POST['sidebar_id'] ) ) $new['sidebar_id'] = sanitize_text_field( $_POST['sidebar_id'] );
	if ( isset( $_POST['cd_content_id'] ) ) $new['cd_content_id'] = sanitize_text_field( $_POST['cd_content_id'] );
	if ( isset( $_POST['cd_page_id'] ) ) $new['cd_page_id'] = sanitize_text_field( $_POST['cd_page_id'] );
	if ( isset( $_POST['cd_page_css'] ) ) $new['cd_page_css'] = sanitize_text_field( $_POST['cd_page_css'] );
	if ( isset( $_POST['dl_container'] ) ) $new['dl_container'] = sanitize_text_field( $_POST['dl_container'] );
	if ( isset( $_POST['dl_items'] ) ) $new['dl_items'] = sanitize_text_field( $_POST['dl_items'] );
	if ( isset( $_POST['dl_pagination'] ) ) $new['dl_pagination'] = sanitize_text_field( $_POST['dl_pagination'] );
	if ( isset( $_POST['dl_next'] ) ) $new['dl_next'] = sanitize_text_field( $_POST['dl_next'] );
	if ( isset( $_POST['show_similar_cars'] ) ) $new['show_similar_cars'] = sanitize_text_field( $_POST['show_similar_cars'] );
	if ( isset( $_POST['search_form_count'] ) ) $new['search_form_count'] = sanitize_text_field( $_POST['search_form_count'] );
	//= Hide Car Demon updates
	if ( isset( $_POST['hide_updates'] ) ) $new['hide_updates'] = sanitize_text_field( $_POST['hide_updates'] );

	if ( isset( $_POST['use_session'] ) ) $new['use_session'] = sanitize_text_field( $_POST['use_session'] );
		...
		echo '<div class="tab" id="specs_tab">';
			...
			echo '<fieldset class="cd_admin_group">';
				...
				echo '<blockquote>';
					...
					//= Add our nonce field
					$nonce = wp_create_nonce( 'update_default_labels_nonce' );
					echo '<input type="hidden" name="update_default_labels_nonce" id="update_default_labels_nonce" value="' . $nonce . '" />';
			...
			//= Add our nonce field
			$nonce = wp_create_nonce( 'option_group_nonce' );
			echo '<input type="hidden" name="option_group_nonce" id="option_group_nonce" value="' . $nonce . '" />';
				...
				echo '<blockquote>';
					foreach ( $specs_options as $group => $value ) {
						...
						echo '<div id="group_' . $group_slug . '">';
							echo '<input type="text" value="' . $group . '" class="vehicle_option_group" id="vehicle_option_group_' . $group_slug . '" />';
							...
							echo '<textarea class="vehicle_option_group_items" id="vehicle_option_group_items_' . $group_slug . '">';
								echo $value;
							echo '</textarea>';
							echo '<div class="clear"></div>';
							echo '<input type="button" value="Update Group" class="btn_update_group" onclick="update_option_group(\'specs\',\'' . $group_slug . '\')" />';
					...
					echo '<blockquote>';
						foreach ( $safety_options as $group => $value ) {
							...
							echo '<div id="group_' . $group . '">';
								echo '<input type="text" value="' . $group . '" class="vehicle_option_group" id="vehicle_option_group_' . $group_slug . '" />';
								echo '<div class="delete_vehicle_option_group" onclick="remove_option_group(\'safety\',\'' . $group_slug . '\')">X - Delete this group</div>';
								echo '<div class="clear"></div>';
								echo '<textarea class="vehicle_option_group_items" id="vehicle_option_group_items_' . $group_slug . '">';
									echo $value;
								echo '</textarea>';
								echo '<div class="clear"></div>';
								echo '<input type="button" value="Update Group" class="btn_update_group" onclick="update_option_group(\'safety\',\'' . $group_slug . '\')" />';
								echo ' <span class="cd_open_caps" data-type="' . $group_slug . '" title="' . __('Manage Capabilities', 'car-demon' ) . '"> + </span>';
								echo cd_specs_cap( $group_slug, $cd_spec_caps );
								echo '<div class="clear"></div>';
							echo '</div>';
					...
					echo '<blockquote>';
						foreach ( $entertainment_options as $group => $value ) {
							...
							echo '<div id="group_' . $group . '">';
								...
								echo '</textarea>';
								echo '<div class="clear"></div>';
								echo '<input type="button" value="Update Group" class="btn_update_group" onclick="update_option_group(\'entertainment\',\'' . $group_slug . '\')" />';
								echo ' <span class="cd_open_caps" data-type="' . $group_slug . '" title="' . __('Manage Capabilities', 'car-demon' ) . '"> + </span>';
								echo cd_specs_cap( $group_slug, $cd_spec_caps );
								echo '<div class="clear"></div>';
							echo '</div>';
					...
					echo '<blockquote>';
						foreach ( $about_us_options as $group => $value ) {
							...
							echo '<div id="group_' . stripslashes_deep( $group ) . '">';
								echo '<input type="text" value="' . stripslashes_deep( $group ) . '" class="vehicle_option_group" id="vehicle_option_group_' . stripslashes_deep( $group_slug ) . '" />';
								echo '<div class="delete_vehicle_option_group" onclick="remove_option_group(\'about_us\',\'' . $group . '\')">X - Delete this group</div>';
								echo '<div class="clear"></div>';
								echo '<textarea class="vehicle_option_group_items" id="vehicle_option_group_items_' . stripslashes_deep( $group_slug ) . '">';
									echo stripslashes_deep( $value );
								echo '</textarea>';
								echo '<div class="clear"></div>';
								echo '<input type="button" value="Update Group" class="btn_update_group" onclick="update_option_group(\'about_us\',\'' . $group_slug . '\')" />';
								echo ' <span class="cd_open_caps" data-type="' . $group_slug . '" title="' . __('Manage Capabilities', 'car-demon' ) . '"> + </span>';
								echo cd_specs_cap( $group_slug, $cd_spec_caps );
								echo '<div class="clear"></div>';
							echo '</div>';
...
function add_vehicle_option_group_form( $group ) {
	...
	$x = '<div id="frm_add_' . $slug . '" class="add_vehicle_option_group_form">';
		$x .= '<blockquote>';
			$x .= 'Group Title:<br /><input type="text" id="group_option_title_' . $slug . '" class="vehicle_option_group" />';
			$x .= '<span id="cancel_' . $slug . '" class="cancel_add_group">- Cancel Add</span>';
			$x .= '<div class="clear"></div>';
			$x .= 'Group Items:<br /><textarea class="vehicle_option_group_items" id="group_options_' . $slug . '">';
			$x .= '</textarea>';
			$x .= '<div class="clear"></div>';
			if ( $group != 'about_us' ) {
		...
		$x .= '<input type="button" value="Add New Group" onclick="add_option_group(\'' . $slug . '\');" />';
	$x .= '</div>';
	return $x;
}
...
function car_demon_remove_option_group() {
	...
	$nonce = $_POST['nonce'];
	if ( ! wp_verify_nonce( $nonce, 'option_group_nonce' ) ) {
		echo __( 'Nonce check failed - no changes saved.', 'car-demon' );
		exit();
	}
...
function car_demon_update_default_labels() {
	...
	$nonce = $_POST['nonce'];
	if ( ! wp_verify_nonce( $nonce, 'update_default_labels_nonce' ) ) {
		echo __( 'Nonce check failed - no changes saved.', 'car-demon' );
		exit();
	}
