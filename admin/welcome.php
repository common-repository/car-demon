			...
			<div class="cd_welcome_tab" id="cd_startup_guide">
				<h3>'.__('Start up guide', 'car-demon').'</h3>
                    ...
                    <p class="cd_setup_inventory">
                        ...
                        <br /><br />';
						$nonce = wp_create_nonce( 'cd_create_inventory_nonce' );
						$html .= '<input type="hidden" name="cd_create_inventory_nonce" id="cd_create_inventory_nonce" value="' . $nonce . '" />';
                        $html .= '<input type="checkbox" name="create_inventory_search" class="create_inventory_search" value="yes" checked="checked" /> ';
						$html .= __('Add a search form to inventory page', 'car-demon') . '
                        <br /><br />
                    ...
                    <p class="cd_setup_text">
						<blockquote>
							...
							<p class="cd_setup_text">
								...
								$nonce = wp_create_nonce( 'cd_insert_samples_nonce' );
								$html .= '<input type="hidden" name="cd_insert_samples_nonce" id="cd_insert_samples_nonce" value="' . $nonce . '" />';
							$html .= '</p>
...
function create_inventory_page() {
	...
	if ( ! current_user_can( 'manage_options' ) ) {
		echo __( 'You do not have permission to do this.', 'car-demon' );
		exit();
	}

	$nonce = $_POST['nonce'];
	if ( ! wp_verify_nonce( $nonce, 'cd_create_inventory_nonce' ) ) {
		echo __( 'Nonce check failed - page not created.', 'car-demon' );
		exit();
	}
