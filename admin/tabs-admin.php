...
function get_tab_specs_admin( $vin_query_decode, $vehicle_vin, $post_id ) {
	...
	//= Add our nonce field
	$nonce = wp_create_nonce( 'cd_edit_vehicle_nonce' );
	$x = '<input type="hidden" name="cd_edit_vehicle_nonce" id="cd_edit_vehicle_nonce" value="' . $nonce . '" />';
...
function custom_spec_field_admin( $post_id, $field, $slug, $odd_even, $vin_query_decode, $disable ) {
	...
	if ( empty( $value ) ) {
		$meta_field = str_replace( 'decoded_', '', $slug );
