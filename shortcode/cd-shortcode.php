...
function cd_show_inventory( $atts ) {
	...
	ob_end_clean();
		...
		if ( $car_query->have_posts() ) : while ( $car_query->have_posts() ) : $car_query->the_post();
			...
			$no_results = '<div class="cd_no_results_msg">';
				$no_results .= __( 'No vehicle results found', 'car-demon' );
			$no_results .= '</div>';
			$no_results = apply_filters( 'cd_no_results_filter', $no_results );
			$html .= $no_results;
		...
		if ( isset( $atts['hide_nav'] ) ) {
			if ( $atts['hide_nav'] == false ) {
				...
				$nav_bottom = str_replace( 'cd-navigation', 'cd-navigation inventory_nav_bottom', $nav_bottom );
