...
function car_demon_search_form( $settings = array() ) {
    ...
    if ( ! isset( $settings['button_text'] ) ) {
        $settings['button_text'] = __( 'Search', 'car-demon' );
    }
    ...
    $class = '';
    if ( defined( 'CD_SEARCH_NARROW' ) ) {
        $class = 'search_car_box_narrow';
    }
    if ( defined( 'CD_SEARCH_CLASS' ) ) {
        $class = CD_SEARCH_CLASS;
    }

	ob_start();
		?>
		<div class="search_car_box_frame">
			<div id="car-demon-search-cars" class="search_car_box<?php echo $class; ?>">
...
<input type="hidden" name="search_condition" value="<?php echo $search_condition;?>" />
		...
		<div id="car-demon-searchr8c1" class="search_body">
			<div id="car-demon-searchr3c1" class=""><?php echo $field_labels['body_style']; ?>:</div>
            <?php echo car_demon_search_body(); ?>
        </div><!--end body-->
		<div id="car-demon-searchr8c2" class="search_button_box">
			<?php
                echo '<div class="advanced_search_btn" title="Advanced Search">+</div>';
                echo '<div class="advanced_search" id="advanced_search"><label>'. __ ('Stock #:', 'car-demon' ) . '</label><input class="search_dropdown_sm" type="text" name="stock" id="stock" size="6" />
            ...
            ?>
		  <input type="submit" name="submit_search" id="submit_search" value="<?php echo $settings['button_text']; ?>" class="search_btn simple_btn">
		</div><!--end search button-->
</form>
	</div>
</div>
...
function car_demon_get_searched_by( $result_page='' ) {
	...
	if ( isset( $_GET['search_location'] ) ) {
		if ( $_GET['search_location'] ) {
			$searched .= '<span class="search_by_item">';
				$searched .= '<span class="remove_search" onclick="remove_search(\'search_location\', \'' . sanitize_text_field( $_GET['search_location'] ) . '\', \'' . $query_string . '\', \'' . $result_page . '\');">x</span> <span class="remove_search_title">' . __( 'Location', 'car-demon' ) . ':</span> ';
				$searched .= stripslashes_deep( sanitize_text_field( $_GET['search_location'] ) ) . '<span class="search_by_comma">,</span>';
			$searched .= '</span>';
		}
	}
	if ( isset( $_GET['search_condition'] ) ) {
	...
	if ( isset( $_GET['search_year_min'] ) ) {
		if ( $_GET['search_year_min'] ) {
			$searched .= '<span class="search_by_item">';
				$searched .= '<span class="remove_search" onclick="remove_search(\'search_year_min\', \'' . sanitize_text_field( $_GET['search_year_min'] ) . '\', \'' . $query_string . '\', \'' . $result_page . '\');">x</span> <span class="remove_search_title">' . __( 'Min Year', 'car-demon' ) . ':</span> ';
				$searched .= stripslashes_deep( sanitize_text_field( $_GET['search_year_min'] ) ) . '<span class="search_by_comma">,</span>';
			$searched .= '</span>';	
		}
	}
	if ( isset( $_GET['search_year_max'] ) ) {
		if ( $_GET['search_year_max'] ) {
			$searched .= '<span class="search_by_item">';
				$searched .= '<span class="remove_search" onclick="remove_search(\'search_year_max\', \'' . sanitize_text_field( $_GET['search_year_max'] ) . '\', \'' . $query_string . '\', \'' . $result_page . '\');">x</span> <span class="remove_search_title">' . __( 'Max Year', 'car-demon' ) . ':</span> ';
				$searched .= stripslashes_deep( sanitize_text_field($_GET['search_year_max']) ) . '<span class="search_by_comma">,</span>';
			$searched .= '</span>';
		}
	}

	if ( isset( $_GET['search_price_min'] ) ) {
		if ($_GET['search_price_min'] ) {
			$searched .= '<span class="search_by_item">';
				$searched .= '<span class="remove_search" onclick="remove_search(\'search_price_min\', \'' . sanitize_text_field( $_GET['search_price_min']) . '\', \'' . $query_string . '\', \'' . $result_page . '\');">x</span> <span class="remove_search_title">' . __( 'Min Price', 'car-demon') . ':</span> ';
				$searched .= stripslashes_deep( sanitize_text_field( $_GET['search_price_min'] ) ) . '<span class="search_by_comma">,</span>';
			$searched .= '</span>';
		}
	}
	if ( isset( $_GET['search_price_max'] ) ) {
		if ($_GET['search_price_max'] ) {
			$searched .= '<span class="search_by_item">';
				$searched .= '<span class="remove_search" onclick="remove_search(\'search_price_max\', \'' . sanitize_text_field( $_GET['search_price_max'] ) . '\', \'' . $query_string . '\', \'' . $result_page . '\');">x</span> <span class="remove_search_title">'. __( 'Max Price', 'car-demon' ) . ':</span> ';
				$searched .= stripslashes_deep( sanitize_text_field( $_GET['search_price_max'] ) ) . '<span class="search_by_comma">,</span>';
			$searched .= '</span>';
		}
	}
	if ( isset( $_GET['search_mileage_min'] ) ) {
		if ($_GET['search_mileage_min'] ) {
			$searched .= '<span class="search_by_item">';
				$searched .= '<span class="remove_search" onclick="remove_search(\'search_mileage_min\', \'' . sanitize_text_field( $_GET['search_mileage_min'] ) . '\', \'' . $query_string . '\', \'' . $result_page . '\');">x</span> <span class="remove_search_title">'. __( 'Min Mileage', 'car-demon' ) . ':</span> ';
				$searched .= stripslashes_deep( sanitize_text_field( $_GET['search_mileage_min'] ) ) . '<span class="search_by_comma">,</span>';
			$searched .= '</span>';
		}
	}
	if ( isset( $_GET['search_mileage_max'] ) ) {
		if ( $_GET['search_mileage_max'] ) {
			$searched .= '<span class="search_by_item">';
				$searched .= '<span class="remove_search" onclick="remove_search(\'search_mileage_max\', \'' . sanitize_text_field( $_GET['search_mileage_max'] ) . '\', \'' . $query_string . '\', \'' . $result_page . '\');">x</span> <span class="remove_search_title">'. __( 'Max Mileage', 'car-demon' ) . ':</span> ';
				$searched .= stripslashes_deep( sanitize_text_field( $_GET['search_mileage_max'] ) ) . '<span class="search_by_comma">,</span>';
			$searched .= '</span>';
		}
	}
	if ( isset( $_GET['search_transmission'] ) ) {
		if ( $_GET['search_transmission'] ) {
			$searched .= '<span class="search_by_item">';
				$searched .= '<span class="remove_search" onclick="remove_search(\'search_transmission\', \'' . sanitize_text_field( $_GET['search_transmission'] ) . '\', \'' . $query_string . '\', \'' . $result_page . '\');">x</span> <span class="remove_search_title">'. __( 'Transmission', 'car-demon' ) . ':</span> ';
				$searched .= stripslashes_deep( sanitize_text_field( $_GET['search_transmission'] ) ) . '<span class="search_by_comma">,</span>';
			$searched .= '</span>';
		}
	}
	if ( isset( $_GET['search_mileage'] ) ) {
		if ($_GET['search_mileage'] ) {
			$searched .= '<span class="search_by_item">';
				$searched .= '<span class="remove_search" onclick="remove_search(\'search_mileage\', \'' . sanitize_text_field( $_GET['search_mileage'] ) .'\', \'' . $query_string . '\', \'' . $result_page . '\');">x</span> <span class="remove_search_title">'. __( 'Miles', 'car-demon' ) .':</span> ';
				$searched .= stripslashes_deep( sanitize_text_field( $_GET['search_mileage'] ) ) . '<span class="search_by_comma">,</span>';
			$searched .= '</span>';
		}
	}
	if ( isset( $_GET['search_body_style'] ) ) {
		if ($_GET['search_body_style'] ) {
			$searched .= '<span class="search_by_item">';
				$searched .= '<span class="remove_search" onclick="remove_search(\'search_body_style\', \'' . sanitize_text_field( $_GET['search_body_style'] ) .'\', \'' . $query_string . '\', \'' . $result_page . '\');">x</span> <span class="remove_search_title">'. __( 'Body Style', 'car-demon' ) .':</span> ';
				$searched .= stripslashes_deep( sanitize_text_field( $_GET['search_body_style'] ) ) . '<span class="search_by_comma">,</span>';
			$searched .= '</span>';
		}
	}
	$searched .= '@@';
