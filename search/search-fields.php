...
function car_demon_search_years() {
	...
	$search_years .= '<select class="search_year" name="search_year" id="search_year">';
		$search_years .= '<option value="">' . __( 'YEAR', 'car-demon' ) . '</option>';
...
function car_demon_search_makes() {
	...
	$search_makes .= '<select class="search_make" name="search_make" id="search_make">';
		$search_makes .= '<option value="">' . __( 'MAKE', 'car-demon' ) . '</option>';
...
function car_demon_search_models() {
	...
	$search_models .= '<select class="search_model" name="search_model" id="search_model" onchange="car_demon_fix_model();">';
		$search_models .= '<option value="">' . __( 'MODEL', 'car-demon' ) . '</option>';
...
function car_demon_search_condition( $settings = array() ) {
	$search_condition = '<select class="search_condition" name="search_condition" id="search_condition">';
		$search_condition .= '<option value="">' . __( 'CONDITION', 'car-demon' ) . '</option>';
...
function car_demon_search_tran() {
	...
	if ( isset( $_GET['search_transmission'] ) ) {
		$current_trans = $_GET['search_transmission'];
	} else {
		$current_trans = '';
	}
	$prefix = $wpdb->prefix;
	...
	if ( ! empty( $trans ) ) {
		$x = '<select id="search_transmission" name="search_transmission" class="search_dropdown_sm">';
...
function car_demon_search_miles() {
	if ( isset( $_GET['search_mileage'] ) ) {
		$mileage = $_GET['search_mileage'];
	} else {
		$mileage = '';
	}
	$x = '<select id="search_mileage" name="search_mileage" class="search_dropdown_sm">';
...
function car_demon_search_body() {
	...
	$label = $field_labels['body_style'];
	$x = car_demon_get_terms_dropdown( $taxonomies, $args, $label );
	return $x;
}
function car_demon_get_terms_dropdown( $taxonomies, $args, $default = '' ) {
	if ( isset( $_GET['search_body_style'] ) ) {
		$body = $_GET['search_body_style'];
	} else {
		$body = '';
	}
	$myterms = get_terms( $taxonomies, $args );
	$output = '<select id="search_body_style" name="search_body_style" class="search_dropdown_sm">';
		if ( empty( $default ) ) {
			$output .= '<option value="">' . __( 'Any', 'car-demon' ) . '</option>';
		} else {
